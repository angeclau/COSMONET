...
CosmonetTraining <- function(x1,y1,screenVars,family="Cox",penalty=c("Fastcox","Coxnet","ADMMnet"),alpha=NULL,Omega=NULL,ncv=NULL){
  ...
  penalty <- match.arg(penalty)
    ...
    if(family=="Cox") {
      fitTrain <- switch(penalty,
                    "Fastcox"=Fastcox(x1[,indexScreen],y1,alpha,ncv),
                    "Coxnet"=Coxnet(x1[,indexScreen],y1,Omega[indexScreen,indexScreen],alpha,ncv),
                    "ADMMnet"=ADMMnet(x1[,indexScreen],y1,Omega[indexScreen,indexScreen],alpha,ncv))
