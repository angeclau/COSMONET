...
validationTest <- function(x2,y2,beta,opt.cutoff){
  ...
  PI.test <- x2 %*% beta
  groupRisk <- vector(mode = "numeric", length = dim(x2)[1]);

  for (i in c(1:nrow(PI.test))){
    if(PI.test[i] >= opt.cutoff){groupRisk[i] <- 1}
    else if(PI.test[i] < opt.cutoff){groupRisk[i] <- 2}
    #print(i)
  }
  ...
  if(length(groupRisk == 1)==0 | length(groupRisk == 2)==0){
