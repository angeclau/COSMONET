...
Coxnet <- function(x1,y1,alpha=NULL,Omega=NULL,ncv=NULL){
    ...
    for(i in 1:ncv){
          # print(i)
            cv <-  Coxnet::Coxnet(x = x1, y = y1, Omega = Omega, penalty = "Net",
                 ...
                 abline(v=log(cv$lambda.max), col="black",lwd=3, lty=2)
  ...
  fit <- Coxnet::Coxnet(x = x1, y = y1, Omega = Omega, penalty = "Net",
                alpha = opt.tuningPars$alpha, lambda=opt.tuningPars$lambda.mean)
